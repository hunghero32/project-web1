...
function list_cv($level,$age, $major, $exp,$address, $salary, $skill)
{
        ...
        $sql .= $skill !== '' ? " AND s.skill LIKE '%" . $skill . "%' " : "";
...
function info_cv($id)
{
        $sql = "SELECT cv.*, u.name, u.email, u.phone, u.address,s.skill,s.percent,ec.corp,ec.start,ec.end,ec.job,d.school,d.year,
            TIMESTAMPDIFF(YEAR, CURRENT_DATE, cv.birth) as age,
            TIMESTAMPDIFF(MONTH, MIN(ec.start), MAX(ec.end)) as exp
            FROM cv
            INNER JOIN user u ON cv.iduser = u.id
            ...
            GROUP BY cv.id, u.name, u.email, u.phone, u.address, cv.birth,s.skill,ec.idcv";
